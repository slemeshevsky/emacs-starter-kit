...
(add-to-list 'semantic-lex-c-preprocessor-symbol-file (concat qt4-base-dir "/Qt/qglobal.h"))
   #+end_src
2. Настройка CEDET для работы с библиотекой ITK-4.5
  #+begin_src emacs-lisp
...
(semantic-add-system-include (concat itk-base-dir "/vnl") 'c-mode)
   #+end_src
3. Настройка CEDET для работы с библиотекой VTK-5.8
   #+begin_src emacs-lisp
...
(ecb-layout-switch "right1")
   #+end_src
8. Комбинация клавиш для быстрой активации/деактивации ECB, 
   #+begin_src emacs-lisp
...
#+end_src
* Навигация по коду с помощью =helm-gtags=
** Включение и настройка ~helm-gtags~
  #+begin_src emacs-lisp
(require 'helm)
(require 'helm-config)
(setq
...
(define-key helm-gtags-mode-map (kbd "C-M-n") 'forward-sexp)
(define-key helm-gtags-mode-map (kbd "C-M-b") 'backward-sexp)
  #+end_src
...
* Настройка окна компиляции
  1. Размер окна компиляции в 10 строк
     #+begin_src emacs-lisp
(setq compilation-window-height 10)
     #+end_src
  2. Автоматически закрывать окно компиляции, если компиляция прошла
     без ошибок
	 #+begin_src emacs-lisp
